...
class GeneralFunctions
{
	...
	public static function getNbaTeam(){
	global $conn;
	
	$path=BASE_PATH."timthumb.php?src=uploads/";
	
	$sql="select id,team_name,CONCAT('$path',team_logo) as team_logo,CONCAT('$path',team_bkwd_logo) as team_bkwd_logo from nba_team_list";
	$sth=$conn->prepare($sql);
	try{$sth->execute();}
	catch(Exception $e){}
	$result=$sth->fetchAll(PDO::FETCH_ASSOC);
