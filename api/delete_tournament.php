...
if(!($token && $event_id && $tournament_id)){
	$success="0";
...
// +-----------------------------------+
	...
	$sth=$conn->prepare("DELETE FROM event_tournament where id=:id and user_id=:user_id and event_id=:event_id");
	$sth->bindValue("event_id",$event_id);
	$sth->bindValue("id",$tournament_id);	
